...
const gradeNames = ['小学1年生', '小学2年生', '小学3年生', '小学4年生', '小学5年生', '小学6年生', '中学1〜2年生', '中学3年生', '常用漢字', '常用外漢字'];
const akasataNames = ['あ〜お', 'か〜こ', 'さ〜そ', 'た〜と', 'な〜の', 'は〜ほ', 'ま〜も', 'や〜よ', 'ら〜り', 'わ〜ん'];

function getGrade(word) {
  var maxGrade = 0;
  for (var i=0; i<word.length; i++) {
    var grade = 0;
    for (var j=1; j<gradeByKanjis.length; j++) {
      if (gradeByKanjis[j].includes(word[i])) {
        grade = j;
        break;
      }
    }
...
function generateAiueo() {
  var arr = new Array(12435 - 12354 + 1);
  for (var i=12354; i<=12435; i++) {
    arr[i-12354] = String.fromCodePoint(i);
  }
  return arr;
}
...
function toSection(mode, variants) {
  ...
  variants.forEach(variant => {
    var [yomi, words] = variant;
    var def = toDef(yomi, words, mode);
    if (def.length == 0) {
    } else {
      html += def;
      count += 1;
...
function toLink(word) {
  var html = '\n';
  var url = 'https://www.google.com/search?q=' + word + 'とは';
  html += '<a href="' + url + '" target="_blank" rel="noopener noreferer">' + word + '</a>\n';
  return html;
}
...
var variants = [];
readEachLineSync('spelling-variants.csv', 'utf8', (line) => {
  var arr = line.split(',');
  var yomi = arr[0];
...
for (var i=1; i<akasatana.length; i++) {
  for (var j=pos; j<variants.length; j++) {
...
for (var i=0; i<index.length-1; i++) {
  for (var j=index[i]; j<index[i+1]; j++) {
    var yomi = variants[j][0];
    var words = variants[j][1];
    for (var g=1; g<gradeByKanjis.length + 1; g++) {
      var candidates = [];
      words.forEach(word => {
        if (getGrade(word) <= g) {
...
for (var g=1; g<learnedKanjis.length + 1; g++) {
  var dir = 'src/' + dirNames[g-1];
  if (!fs.existsSync(dir)) {
    fs.mkdirSync(dir);
  }
  for (var i=0; i<akasataNames.length; i++) {
    var akasataDir = dir + '/' + akasataNames[i];
    if (!fs.existsSync(akasataDir)) {
      fs.mkdirSync(akasataDir);
    }
    var html = ejs.render(template, {
      grade:g, gradeName:gradeNames[g-1], variants:data[g][i], akasataName:akasataNames[i],
      gradeNames:gradeNames, akasataNames:akasataNames, dirNames:dirNames,
      toNav:toNav, toSection:toSection, selected:selected,
    });
    fs.writeFileSync(akasataDir + '/index.html', html);
  }
}
